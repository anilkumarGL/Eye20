package com.mymarketapps.eye20.utils;

/**
 * Created by Anil Kumar on 1/6/15.
 */
...
public class AnimationFactory {
    ...
    public static Animation[] flipAnimation(final View fromView, final View toView, FlipDirection dir, long duration, Interpolator interpolator) {
        ...
        centerX = fromView.getMeasuredWidth() / 2.0f;
        centerY = fromView.getMeasuredHeight() / 2.0f;
     ...
     */
    public static void flipTransition(final ViewAnimator viewAnimator, FlipDirection dir, AnimationListener listener) {

        final View fromView = viewAnimator.getCurrentView();
        final int currentIndex = viewAnimator.getDisplayedChild();

        Random mRandom = new Random();
        int nextIndex = mRandom.nextInt(viewAnimator.getChildCount());

        if (nextIndex == currentIndex) {
            nextIndex = nextIndex+1 == viewAnimator.getChildCount() ? nextIndex-1 : nextIndex+1;
        }

        final View toView = viewAnimator.getChildAt(nextIndex);

        Animation[] animc = AnimationFactory.flipAnimation(fromView, toView, dir, 200, null);
        ...
        animc[1].setAnimationListener(listener);
    }
     ...
     * @return a slide transition animation
     */
    public static Animation outToLeftAnimation(long duration, Interpolator interpolator, float toDeltaX) {
        Animation outtoLeft = new TranslateAnimation(
                Animation.ABSOLUTE,  0.0f, Animation.ABSOLUTE,  -1.0f*toDeltaX,
    ...
    public static void translateAnimation(View v, AnimationListener listener, float fromDeltaX, float toDeltaX, float fromDeltaY, float toDeltaY) {

        TranslateAnimation ta = new TranslateAnimation(fromDeltaX, toDeltaX, fromDeltaY, toDeltaY);
        ta.setDuration(500);
